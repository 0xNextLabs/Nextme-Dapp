...
export type ProgressConstructor = { progress?: number; id: string; label?: string }

export type ProgressOnChangeProps = { key: string; progress: string; items: Progress[] }
...
export class ProgressItem {
  ...
  constructor(
    readonly key: string,
    config?: ProgressConstructor[]
  ) {
    if (config && Array.isArray(config)) {
      for (const item of config) {
        this._items.set(item.id, { progress: item.progress || 0, label: item.label })
      }
    }
  }
  public get items() {
  ...
  public setProgress(key: string, progress: number) {
    ...
    if (item) {
      ...
      this._event.emit('change', { key, progress, items: Array.from(this._items.values()) })
      if (Array.from(this._items.entries()).every(item => item[1].progress >= 100)) {
        this.complete = true
...
export class ProgressContainer {
  ...
  public createProgress(props: { steps: ProgressConstructor | ProgressConstructor[]; key?: string }) {
...
export const ProgressStore = React.createContext<ProgressContainer | null>(null)

export const useProgressStore = () => {
  return useContext(ProgressStore)
}

export const ProgressContainerProvider: FC<PropsWithChildren<{}>> = ({ children }) => {
  return <ProgressStore.Provider value={progressInstance}>{children}</ProgressStore.Provider>
}

export const ProgressView: FC<{ onClose: () => void; progress?: ProgressItem; isMobile?: boolean }> = ({
  onClose,
  progress,
  isMobile = false,
}) => {
  ...
  return (
    <div
      className={classNames(
        ...
        'min-h-56 border p-4 flex flex-col justify-around bg-white/80 backdrop-blur gap-2 shadow-xl'
      )}
    >
      <div className="w-full chat chat-end">
        <div className="chat-image avatar">
          <div className="w-10 rounded-full">
            <Avatar src={config.logo.dark} />
          </div>
        </div>
      ...
      <div className="flex flex-col justify-around items-center flex-1 gap-3 w-full">
        {items.map((item, key) => {
          return (
            <div key={key} className="w-full flex flex-col gap-2 justify-center items-start">
              <div className="text-ellipsis overflow-hidden flex-1 whitespace-nowrap text-xs">{item.label}</div>
              <div className="flex justify-center items-center gap-2 w-full">
                <progress
                  className={classNames(
                    'progress flex-1',
                    item.progress == 100 ? 'progress-success' : 'progress-warning'
                  )}
                  value={item.progress}
                  max="100"
                />
...
export const progressCounter = (progress: ProgressItem | undefined, key: string, interval = 100) => {
  if (progress) {
    let _max = 0
